...
import ru.java.crm.repository.ClientRepository;
import ru.java.crm.repository.ManagerRepository;
import ru.java.crm.service.DBServiceClient;
import ru.java.crm.service.DBServiceManager;
...
@Component("serviceForDemo")
public class ServiceForDemo {
    private static final Logger log = LoggerFactory.getLogger(ServiceForDemo.class);
...
//// создаем Manager
        ...
        var managerSecond = dbServiceManager.saveManager(new Manager(
                ...
                "ManagerSecond",
                Set.of(
                        new Client("managClient1", managerName, 1),
                        new Client("managClient2", managerName, 2)
                ),
                new ArrayList<>(),
                true
        ));
...
/// применяем переопределенные методы репозиториев
