...
public class PhoneRepositoryImpl implements PhoneRepository {
    ...
    public <T> void insert(T value, Class<T> tClass) {
        ...
        if (tClass.equals(Phone.class)) {
            ...
            session.execute(
                    ...
                    phone.getId(), phone.getModel(), phone.getColor(), phone.getSerialNumber()
            );
            ...
            session.execute(
                    "INSERT INTO Products.Phones (id, model, color, serialNumber, operatingSystem) " + "VALUES (?, ?, ?, ?, ?)",
                    phone.getId(), phone.getModel(), phone.getColor(), phone.getSerialNumber(), phone.getOperatingSystem()
            );
    ...
    public <T> List<T> findAll(Class<T> tClass) {
        ...
        CqlSession session = cassandraConnection.getSession();
        // Запрос ужасен, поскольку для его выполнения необходимо обойти все ноды во всех датацентрах, что очень непроизводительно
        // Выборку желательно делать либо по ключу, либо по тому, что есть в ключе
