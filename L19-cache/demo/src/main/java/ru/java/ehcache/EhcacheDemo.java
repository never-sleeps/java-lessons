...
public class EhcacheDemo {
    ...
    private void withoutCache() {
        ...
        IntStream.range(1, 10).forEach(val -> logger.info("withoutCache value: {}", SlowDataSource.getValue(val)));
        logger.info("second getting...");
        IntStream.range(1, 10).map(i -> 10 - i).forEach(val -> logger.info("withoutCache value: {}", SlowDataSource.getValue(val)));
    }
    ...
    private long getValue(int key) {
        ...
        if (value == null) {
            value = SlowDataSource.getValue(key);
            cache.put(key, value);
        }
        return value;
    }
